use_frameworks!
platform :ios, '10.0'

inhibit_all_warnings!

target :OpenWallet do
    pod 'TesseractEthereumBase', :git => 'git@github.com:ypopovych/swift-ethereum-base.git', :branch => 'master'
    pod 'SerializableValue', :git => 'https://github.com/ypopovych/swift-serializable.git', :branch => 'master'
    
    pod 'TesseractWallet', :git => 'git@github.com:ypopovych/swift-wallet-sdk.git', :branch => 'master'
    pod 'TesseractWallet/Ethereum', :git => 'git@github.com:ypopovych/swift-wallet-sdk.git', :branch => 'master'
    pod 'TesseractKeychain', :git => 'git@github.com:ypopovych/swift-keychain.git', :branch => 'master'
    
    pod 'PromiseKit', '~> 6.8.0'
end

target :OpenWalletTests do
    pod 'TesseractEthereumBase', :git => 'git@github.com:ypopovych/swift-ethereum-base.git', :branch => 'master'
    pod 'SerializableValue', :git => 'https://github.com/ypopovych/swift-serializable.git', :branch => 'master'
    
    pod 'TesseractWallet', :git => 'git@github.com:ypopovych/swift-wallet-sdk.git', :branch => 'master'
    pod 'TesseractWallet/Ethereum', :git => 'git@github.com:ypopovych/swift-wallet-sdk.git', :branch => 'master'
    pod 'TesseractKeychain', :git => 'git@github.com:ypopovych/swift-keychain.git', :branch => 'master'
    
    pod 'PromiseKit', '~> 6.8.0'
end
